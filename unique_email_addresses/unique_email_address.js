...
const numUniqueEmails = (emails) => {
  // turn each email into a list with local name and domain name
  const splitEmails = emails.map((email) => {
    const result = email.split("@");
    // localname without pluses
    result[0] = result[0].split("+")[0];
    let split = result[0].replace(/\./g, "");
    // localname without .
    result[0] = split;
    return result.join("@");
  });
  return uniq(splitEmails).length;
};
